...
function setRaiting (arr) {                                     // ф-ия для установления рентинга на звёздочках
	...
	setCookie(mass);                                              // вызываем ф-ию для записи куков и передаём туда массив
}

setRaiting(stars);                                              // вызываем ф-ию установления рейтинга на звёздах

function setCookie(arr) {                                       // ф-ия записи кук
	let result = [];
	for (let i = arr.length; i <= arr.length; i++) {                        // перебираем все значения из массива
